...
void LoadMapObjects(const char *path, Point start, Rectangle mapRange, int leveridx)
{
	...
	const uint16_t *objectLayer = &dunData[2 + width * height * 5];
	...
	if (Objects[i]._oSelFlag != 0 && !qtextflag) {
		...
		if (Objects[i]._oAnimFrame != Objects[i]._oVar6) {
			...
			if (Objects[i]._otype == OBJ_BLINDBOOK) {
				...
				auto tren = TransVal;
				TransVal = 9;
				DRLG_MRectTrans({ { Objects[i]._oVar1, Objects[i]._oVar2 }, { Objects[i]._oVar3 - Objects[i]._oVar1, Objects[i]._oVar4 - Objects[i]._oVar2 } });
				TransVal = tren;
			}
		}
...
void OperatePedistal(int pnum, int i)
{
	...
	if (Objects[i]._oVar6 == 3) {
		...
		ObjChangeMap(Objects[i]._oVar1, Objects[i]._oVar2, Objects[i]._oVar3, Objects[i]._oVar4);
		LoadMapObjs("Levels\\L2Data\\Blood2.DUN", SetPiece.position.megaToWorld());
...
void SyncQSTLever(const Object &qstLever)
{
	if (qstLever._oAnimFrame == qstLever._oVar6) {
		...
		if (qstLever._otype == OBJ_BLINDBOOK) {
			auto tren = TransVal;
			TransVal = 9;
			DRLG_MRectTrans({ { qstLever._oVar1, qstLever._oVar2 }, { qstLever._oVar3 - qstLever._oVar1, qstLever._oVar4 - qstLever._oVar2 } });
			TransVal = tren;
		}
	}
}
...
void SyncPedestal(const Object &pedestal, Point origin, int width)
{
	...
	if (pedestal._oVar6 == 3) {
		...
		LoadMapObjs("Levels\\L2Data\\Blood2.DUN", origin.megaToWorld());
	}
}
	...
	if (currlevel == 16) {
		...
		if (leveltype == DTYPE_CATACOMBS) {
			...
			if (Quests[Q_BLIND].IsAvailable()) {
				...
				LoadMapObjs("Levels\\L2Data\\Blind2.DUN", SetPiece.position.megaToWorld());
			}
			if (Quests[Q_BLOOD].IsAvailable()) {
				_speech_id spId;
				switch (MyPlayer->_pClass) {
		...
		if (leveltype == DTYPE_HELL) {
			if (Quests[Q_WARLORD].IsAvailable()) {
				...
				LoadMapObjs("Levels\\L4Data\\Warlord.DUN", SetPiece.position.megaToWorld());
			}
