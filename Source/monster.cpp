...
void PlaceUniqueMonst(int uniqindex, int miniontype, int bosspacksize)
{
	...
	if (uniqindex == UMT_BUTCHER) {
		bool done = false;
		for (position.y = 0; position.y < MAXDUNY && !done; position.y++) {
			for (position.x = 0; position.x < MAXDUNX && !done; position.x++) {
				done = dPiece[position.x][position.y] == 367;
			}
		}
	}
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			...
			if (monsterId != 0) {
				...
				PlaceMonster(ActiveMonsterCount++, mtype, startPosition + Displacement { i + 16, j + 16 });
			}
		}
	}
}
